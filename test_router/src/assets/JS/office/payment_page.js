...
$(() => {
  ...
  // custom-radio action
  // *******************
  const payment_all = parseInt($('#payment_all').attr('payment_all'));
  ...
  $('.use-mileage-btn').click(() => {
    ...
    if ($('.inner-radio:eq(0)').hasClass('choice-radio')) {
      ...
      // 후불 결제 시 로직
      const payment_later = payment_all * 0.2;
  ...
  $('#pay-btn').click(() => {
    const reserve_no = $(location).attr('href').split('reserve_no=')[1];
